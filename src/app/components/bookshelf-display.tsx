...
import { Star, BookOpen, User, Clock, Sparkles, Plus, Trash2, Share2 } from "lucide-react"
...
export default function BookshelfDisplay({
  bookshelf,
  editable = false,
}: { bookshelf: BookshelfProps; editable?: boolean }) {
  ...
  const addBook = () => {
    if (newBook.title && newBook.author) {
      ...
      setBooks((prev) => [...prev, { ...newBook, id: newId, status: "available" }])
  ...
  const lendBook = () => {
    if (selectedBookId && lendToName) {
      setBooks((prev) =>
        prev.map((book) =>
          book.id === selectedBookId ? { ...book, status: "borrowed", borrowedBy: lendToName } : book,
        ),
      )
