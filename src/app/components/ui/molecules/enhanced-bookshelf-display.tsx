...
import { Star, User, BookOpen, Plus, Trash2, Share2, Clock, Sparkles } from "lucide-react"
...
export default function EnhancedBookshelfDisplay({
  bookshelf,
  editable = false,
}: {
  ...
  const addBook = () => {
    if (newBook.title && newBook.author) {
      ...
      setBooks((prev) => [...prev, { ...newBook, id: newId, availability: "available" }])
      setNewBook({ title: "", author: "", rating: 5 })
      setIsAddBookOpen(false)

      toast({
        title: "Book Added",
        description: `"${newBook.title}" has been added to your bookshelf.`,
        duration: 3000,
      })
    }
  }
  ...
  const lendBook = () => {
    if (selectedBookId && lendToName) {
      setBooks((prev) =>
        prev.map((book) =>
          book.id === selectedBookId ? { ...book, availability: "borrowed", borrowedBy: lendToName } : book,
        ),
      )
  ...
  const getPlantDecoration = (theme: string, position: "left" | "right" | "center") => {
  ...
  return (
    <div className="w-full overflow-hidden">
      <div className="relative w-full">
        ...
        >
          {/* Owner information */}
          <div className="absolute top-2 right-3 flex items-center gap-2 bg-white/80 backdrop-blur-sm px-3 py-1 rounded-md shadow-sm z-10">
            <User className="h-4 w-4 text-purple-600" />
            <span className="text-xs font-medium text-purple-800">{bookshelf.owner.name}</span>
          </div>
          ...
          <div className="relative h-[220px] flex items-end">
            {books.map((book, index) => {
              ...
              const tiltAngle = Math.random() > 0.7 ? (Math.random() > 0.5 ? 3 : -3) : 0
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      book.availability === "borrowed" ? "opacity-70" : "",
                    )}
                    style={{
                  ...
                  >
                    ...
                    {isHovered && (
                      <>
                        ...
                        <div
                          ...
                          style={{
                            background: "linear-gradient(to right, rgba(0,0,0,0.2), rgba(255,255,255,0.3))",
                          }}
                        ></div>
                      </>
                    )}
                    ...
                    {isHovered && book.availability === "available" && !editable && (
                      <button
                  ...
                  {isHovered && (
                    <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 translate-y-full w-[220px] bg-white/95 backdrop-blur-sm rounded-md shadow-lg p-3 z-20 border border-purple-100">
                      ...
                      <h4 className="font-medium text-purple-800 text-sm">{book.title}</h4>
                      ...
                      <div className="mt-2 pt-2 border-t border-purple-100">
                        <div className="flex items-center">
                          <div
                            className={cn(
                              ...
                              book.availability === "available" ? "bg-green-500" : "bg-red-500",
                            )}
                          ></div>
                          <span
                            className={cn(
                              "text-xs font-medium",
                              book.availability === "available" ? "text-green-600" : "text-red-600",
                            )}
                          >
                        ...
                        {book.availability === "borrowed" && book.borrowedBy && (
                        ...
                        {book.availability === "borrowed" && book.returnDate && (
                          <div className="text-xs text-purple-600 mt-1 flex items-center">
                            <Clock className="h-3 w-3 mr-1" />
                            <span>Return by: {new Date(book.returnDate).toLocaleDateString()}</span>
                          </div>
                        )}

                        <div className="mt-2">
                          {book.availability === "available" && !editable ? (
                            <button
          ...
          <div className="absolute bottom-0 left-0 right-0 h-12">
            ...
            {/* Shelf bottom edge */}
            <div className={cn("absolute -bottom-1 left-0 right-0 h-2", shelfStyle.edge)}></div>
